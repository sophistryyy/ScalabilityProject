...
public interface MatchingEngineOrdersRepository extends JpaRepository<StockOrder, Integer> {
    ...
    LinkedList<StockOrder> getAllSellByStock_id(Long stock_id);

    @Query("SELECT so FROM StockOrder so WHERE so.stock_id = ?1 AND so.orderStatus != seng468.scalability.models.entity.StockOrder$OrderStatus.COMPLETED " +
            "AND so.expired = false AND so.is_buy = true ORDER BY so.price DESC NULLS FIRST, so.timestamp ASC")
    LinkedList<StockOrder> getAllBuyByStock_id(Long stock_id);
    ...
    StockOrder findByUsernameAndStockTxId(Long stockTxId, String username);

    //rework this!
    @Query("SELECT so FROM StockOrder so WHERE so.stock_id = ?1 AND so.orderStatus = seng468.scalability.models.entity.StockOrder$OrderStatus.COMPLETED" +
            " AND so.price is not null AND so.expired = false AND so.is_buy = false ORDER BY so.timestamp DESC")
    LinkedList<StockOrder> getLastSellOrderByStockId(Long stock_id, Pageable pageable);

    @Query("SELECT so FROM StockOrder so WHERE so.stock_id = ?1 AND so.orderStatus != seng468.scalability.models.entity.StockOrder$OrderStatus.COMPLETED" +
            " AND so.price is not null AND so.expired = false AND so.is_buy = false ORDER BY so.price DESC")
    LinkedList<StockOrder> getLowestSellOrderByStockId(Long stock_id, Pageable pageable);
    ...
    LinkedList<StockOrder> findAllParentChildTransactions(Long parentStockTxId);
}
